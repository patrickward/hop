...
import (
	...
	"github.com/patrickward/hop/chain"
)
	...
	for _, setting := range settings {
		t.Run(setting.Name, func(t *testing.T) {
			...
			assert.NotPanics(t, func() {
				chain.RecoverPanic(setting.render)(setting.handler).ServeHTTP(w, r)
			})
...
func TestSecurityHeaders(t *testing.T) {
	r := httptest.NewRequest(http.MethodGet, "/", nil)
	w := httptest.NewRecorder()
	chain.SecurityHeaders(fakeHandler()).ServeHTTP(w, r)
...
func TestContentSecurityPolicy(t *testing.T) {
	r := httptest.NewRequest(http.MethodGet, "/", nil)
	w := httptest.NewRecorder()
	chain.ContentSecurityPolicy(nil)(fakeHandler()).ServeHTTP(w, r)
...
func TestPreventCSRF(t *testing.T) {
	...
	chain.PreventCSRF(fakeHandler()).ServeHTTP(w, r)
	assert.Equal(t, "fakeHandler", w.Body.String())
}
