...
func Example() {
	...
	tmpfile, err := os.CreateTemp("", "hopconfig.*.json")
	if err != nil {
	...
	cfg := &AppConfig{}
	if err := hopconfig.Load(cfg, tmpfile.Name()); err != nil {
		fmt.Printf("Error loading hopconfig: %v\n", err)
		return
	}
...
func ExampleDuration() {
	type Config struct {
		Timeout hopconfig.Duration `json:"timeout" env:"TIMEOUT" default:"30s"`
	}

	cfg := &Config{}

	// Load with just defaults
	if err := hopconfig.Load(cfg); err != nil {
		fmt.Printf("Error loading hopconfig: %v\n", err)
		return
	}
...
func ExampleBasicFlags() {
	...
	cfg := &AppConfig{}
	if err := hopconfig.Load(cfg, fs.Lookup("hopconfig").Value.String()); err != nil {
		fmt.Printf("Error loading hopconfig: %v\n", err)
		return
	}

	// STEP7: Apply flag overrides
	hopconfig.ApplyFlagOverrides(&cfg.BaseConfig, fs)

	// Apply other flag values that should override config
	if apiTimeout != nil && *apiTimeout != 0 {
		cfg.API.Timeout = hopconfig.Duration{Duration: *apiTimeout}
	}
